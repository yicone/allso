;(function() {
  ...
  var app = {
    ...
    resizePanel: function() {
      ...
      console.log('app._state.show_side on resizePanel', app._state.show_side)
    ...
    doubleSearch: function(side) {
      app.$a[0].src = app.getUrl(0)

      var keywords_a = $(app.$soinput[0]).val()
      var keywords_b = $(app.$soinput[1]).val()
      if (!keywords_a && !keywords_b) return
      ...
      if (app._state.sync_mode == 1) {
        ...
        // 双侧同步模式为普通模式
        if (side == 0 || !keywords_b) {
          $(app.$soinput[1]).val(keywords_a)
        }
      ...
      var translate_url = `https://whitehole.gadn.in/.netlify/functions/gtrans?sl=${sl}&tl=${tl}&dt=t&q=${keywords}`
      $.ajax({
        url: translate_url,
        dataType: 'text',
    ...
    searchLoading: function(side) {
      var $side_progress = side == 2 ? app.$progress : $(app.$progress[side])
      $side_progress.addClass('progress-bar-striped active')
      window.setTimeout(function() {
        $side_progress.removeClass('progress-bar-striped active')
      }, 2000)
      ...
      if (side == 2) {
        if (app._state.show_side == 2) {
          ...
          side = app._state.show_side
        }
      } else if (side == 3) {
        side = 0
      }
      var side_keywords = $.trim(app.$soinput[side].value)
      if (side_keywords) {
        window.document.title = side_keywords + ' - 虫洞搜索'
      } else {
    ...
    // 用例7: 当不指定搜索侧，但如切换同步模式时(side=3)，如之前为单侧模式，则恢复为双侧，双侧同时搜索
    // 用例8: 当不指定搜索侧，但如切换同步模式时(side=3)，如之前为双侧模式，双侧同时搜索
    // 用例9: 当不指定搜索侧(side=2)，沿用之前模式
    startSearch: function(side) {
      ...
      console.log('start search: side', side)
  ...
  $(function() {
    ...
    // 加载搜索引擎列表
    $.get('https://whitehole.gadn.in/.netlify/functions/engines-get-all', function(data) {
      ...
      $engines.each(function(i) {
        for (var j in app._enginesConfig.ENGINES) {
          ...
          var li = `<li><div><img src="${engine.logo}" style="margin:5px;width:32px;height:32px"/><a href="#" data-side="${side}" data-engine-key="${engine_key}">${engine_key}</a></div></li>`
          $(this).append(li)
        }
      })
