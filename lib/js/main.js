...
function EnginesConfig() {
  this.ENGINES = [
    ...
    {
      Twitter: [
        "https://twitter.com/search?q=${k}",
        "https://pbs.twimg.com/profile_images/968232588958560256/IfOqS8l2.jpg",
        -76,
        -120,
        87
      ]
    ...
    {
      Ask: [
        "https://www.search.ask.com/web?q=${k}",
    ...
    {
      Quora: [
        "https://www.quora.com/search?q=${k}",
    ...
    {
      搜狗: [
        "https://www.sogou.com/web?query=${k}",
    ...
    {
      必应: [
        "https://cn.bing.com/search?setmkt=zh-cn&setlang=zh-cn&q=${k}",
    ...
    },
    {
      ...
      极客搜索: [
        "https://s.geekbang.org/search/c=0/k=${k}/t=",
        "http://read.html5.qq.com/image?src=forum&q=5&r=0&imgflag=7&imageUrl=http://mmbiz.qpic.cn/mmbiz_png/8NUG2LLiagCZOc4yRJkDPgAnJgtEQKV8tgicPwwEMtwpYZvicPI3gD8kX0ZsWWU9T9dsUfCso64EVC2C9dEC88LDw/0.png",
        -120,
        0,
        200
      ]
    ...
    {
      Instapaper: [
        "https://www.instapaper.com/search?q=${k}",
    ...
    {
      Evernote: [
        "https://www.evernote.com/Home.action#ses=1&sh=5&sds=5&x=${k}",
    ...
    {
      Wunderlist: [
        "https://www.wunderlist.com/webapp#/search/${k}",
...
function saveState() {
  localStorage["allso_state"] = JSON.stringify(_state);
}
...
var _state = $.extend({}, default_state, _state);
console.log("launch engines:", _state.enginePair);

scale(); //启动后的第一次响应式

$engines.each(function(i) {
  for (var j in _enginesConfig.ENGINES) {
    var engine = _enginesConfig.ENGINES[j];
    var side = i;
...
// 设置初始同步模式
$($(".sync-mode>a")[_state.sync_mode]).addClass("btn-danger");

/* 设置初始搜索引擎 */
for (var i in _state.enginePair) {
  // var unselected = btn_styles[i];
  // var selected = btn_styles[parseInt(i) + 2];
  // var $buttons = $($engines[i]).find('button');

  var engine_key = _state.enginePair[i];
...
function set(side, engine_key) {
  ...
  if (_enginesConfig.exists(engine_key)) {
    _state.enginePair[side] = engine_key;
    saveState();
  } else {
    engine_key = _state.enginePair[side];
  }

  var engine_config = _enginesConfig.get(engine_key);
...
function doSearch(side) {
  ...
  if (keywords_a !== "") {
    ...
    if (_state.show_side !== 2 && side !== _state.show_side) {
      _state.show_side = 2;
      // saveState();
      scale();
    }
...
function change_state(is_zuo) {
  var side = is_zuo ? 0 : 1;
  if (_state.show_side == 2) {
    _state.show_side = side;
    // 单边显示时，取消左偏移
    set_left[side] = 0;
  } else {
    _state.show_side = 2;
  ...
  if (_state.show_side == 2) doSearch();
}
...
function rightSearch() {
  ...
  var sl, tl;
  if (_state.sync_mode == 1) {
    (sl = "en"), (tl = "zh-CN");
  } else if (_state.sync_mode == 2) {
    (sl = "zh-CN"), (tl = "en");
  } else {
    $($soinput[1]).val(keywords);
    $b[0].src = getUrl(1);
...
function getUrl(i) {
  ...
  var url = set_url[i].replace('${k}', encodeURIComponent(keywords));
  if (_state.enginePair[i] == "Evernote") {
    url += "&";
  }
  return url;
}
