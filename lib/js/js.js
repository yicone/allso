...
const $soinput = $('.soinput'),
    ...
    engines = [
        ...
        { "Ask": ["https://www.search.ask.com/web?q=", "https://www.search.ask.com/assets/b/a-logo-small.png", -60, -120, 120] },
        { "Quora": ["https://www.quora.com/search?q=", "https://qsf.ec.quoracdn.net/-3-images.logo.wordmark_default.svg-26-32753849bf197b54.svg", 0, 0, 0] },
        ...
        { "Evernote": ["https://www.evernote.com/Home.action#ses=1&sh=5&sds=5&x=", "https://evernote.com/media/img/brand-guide/evernote-logos/Evernote_H.png", 0, 0, 0] },
        { "Wunderlist": ["https://www.wunderlist.com/webapp#/search/", "https://dr0wv9n0kx6h5.cloudfront.net/664cb69d34d0ef040ff8a446e429bce8feb54b41/site/images/logo-big.png", 0, 0, 0] }
    ];
...
if (localStorage["allso_state"] !== undefined) {
    ...
    var old_state = JSON.parse(data);
    state = $.extend({}, old_state, default_state);
}
...
$engines.each(function (i) {
    for (var j in engines) {
        ...
        var li = `<li><a onclick="set(${side},'${engine_key}');so(${side});">${engine_key}</a></li>`;
        $(this).append(li);
        // todo: 消除该硬编码
...
function set(side, engine_key) {
    if(["Google", "微博", "Evernote"].indexOf(engine_key) >= 0){
        var anwser = confirm(engine_key + " 搜索需要安装 Chrome 插件: X-Frame-Options\n\nhttps://chrome.google.com/webstore/detail/gleekbfjekiniecknbkamfmkohkpodhe\n\n已安装？");
        if(!anwser){
            return;
        }
    }
...
function so(side) {
    ...
    if (keywords_a !== '') {
        ...
        if (side === undefined) {
            ...
            $iframe[side].src = getUrl(side);
        }

        // 防止输入时自动搜索导致浏览器产生多条请求记录
        //location.hash = sowhat_str;
        window.document.title = keywords_a + ' - ALLSO';
    }
    else {
        location.hash = '';
